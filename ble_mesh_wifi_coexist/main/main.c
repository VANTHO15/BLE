...
void example_ble_mesh_send_gen_onoff_set(bool onoff)
{
                                               ...
                                               esp_ble_mesh_generic_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT:
        ...
        ESP_LOGI(TAG, "ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT, onoff %d", param->status_cb.onoff_status.present_onoff);
        break;
...
void app_mqtt_data_callback(char *data, int len)
{
    printf("DATA: %.*s\n", len, data);
    if(strstr(data, "set"))
    {
...
static void wifi_init(void)
{
    ...
    EventBits_t bits = xEventGroupWaitBits(s_wifi_event_group,
            ...
            pdFALSE,
            portMAX_DELAY);  
    ESP_LOGI(TAG, "Wifi connected to Router"); 
    mqtt_app_init();
    mqtt_set_callback(app_mqtt_data_callback);
    mqtt_app_start(); 
}
